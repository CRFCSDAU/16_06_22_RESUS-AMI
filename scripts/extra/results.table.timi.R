...
# pre_timi
  ...
  x <- select(data,
    ...
    select(id, arm, time, bsa.m2, any.diabetes, pre_timi,
           everything())
...
# look <- select(x, id, arm, contains("lvesv"))
#
# gather(look, type, val, -arm, -id) %>%
# group_by(arm, type) %>%
#   summarise(count = n())

# x <- x[order(names(x))]
# x <- select(x, id, arm, any.diabetes, bsa.m2, every)
  ...
  ggplot(x, aes(x = pre_timi, y = lvedv.index_FU, color = pre_timi)) +
    geom_jitter(width = 0.1) +
    facet_wrap(~arm) +
    scale_fill_brewer("", palette = "Set1") +
    theme_minimal()

  ggplot(x, aes(x = as.numeric(pre_timi), y = lvedv.index_FU)) +
    geom_smooth(method = "lm", color = "black", se = FALSE) +
    geom_smooth(color = "black", se = FALSE) +
    geom_jitter(aes(color = arm), width = 0.1, size = 2) +
    scale_fill_brewer("", palette = "Set1") +
    theme_minimal()
  ...
  lvedv      <- lm(lvedv_FU          ~ arm + bsa.m2 + any.diabetes + lvedv_BL + pre_timi,
                   data = x) %>% tidy()
  lvesv      <- lm(lvesv_FU          ~ arm + bsa.m2 + any.diabetes + lvesv_BL + pre_timi,
                   data = x) %>% tidy()
  glvef      <- lm(glvef_FU          ~ arm +          any.diabetes + glvef_BL + pre_timi,
                   data = x) %>% tidy()
  lvmass.a   <- lm(lvmass.a_FU       ~ arm + bsa.m2 + any.diabetes +
                     lvmass.a_BL + pre_timi,
                   data = x) %>% tidy()
  latece     <- lm(latece_FU         ~ arm +          any.diabetes + latece_BL + pre_timi,
                   data = x) %>% tidy()
  sv.main    <- lm(sv.main_FU        ~ arm +          any.diabetes + sv.main_BL + pre_timi,
                   data = x) %>% tidy()
  inf.size   <- lm(inf.size_FU       ~ arm +          any.diabetes +
                     inf.size_BL + pre_timi,
                   data = x) %>% tidy()
  lv.mass.ed <- lm(lv.mass.ed_FU     ~ arm + bsa.m2 + any.diabetes +
                     lv.mass.ed_BL + pre_timi,
                   data = x) %>% tidy()
  lv.mass.es <- lm(lv.mass.es_FU     ~ arm + bsa.m2 + any.diabetes +
                     lv.mass.es_BL + pre_timi,
                   data = x) %>% tidy()
  thickening <- lm(avg.thickening_FU ~ arm +     + any.diabetes +
                     avg.thickening_BL + pre_timi,
                   data = x) %>% tidy()

# A look at whether adjusting for late CE at baseline matters

  latece2     <- lm(latece_FU         ~ arm +   latece_BL+ any.diabetes,
                    data = x) %>% tidy() #yes


# indexed
  lvedv.index      <- lm(lvedv.index_FU      ~ arm + any.diabetes + pre_timi + lvedv.index_BL,
                         data = x) %>% tidy()
  lvesv.index      <- lm(lvesv.index_FU      ~ arm + any.diabetes + pre_timi + lvesv.index_BL,
                         data = x) %>% tidy()
  lvmass.a.index   <- lm(lvmass.a.index_FU   ~ arm + any.diabetes + pre_timi + lvmass.a.index_BL,
                         data = x) %>% tidy()
  lv.mass.ed.index <- lm(lv.mass.ed.index_FU ~ arm + any.diabetes + pre_timi + lv.mass.ed.index_BL,
                         data = x) %>% tidy()
  lv.mass.es.index <- lm(lv.mass.es.index_FU ~ arm + any.diabetes + pre_timi + lv.mass.es.index_BL,
                         data = x) %>% tidy()
...
# model results
  ...
  b.ll <- c(lvedv$estimate[2] - (1.96 * lvedv$std.error[2]),
            ...
            latece2$estimate[2] - (1.96 * latece2$std.error[2]),
  ...
  c.ll <- c(lvedv$estimate[3] - (1.96 * lvedv$std.error[3]),
            ...
            latece2$estimate[3] - (1.96 * latece2$std.error[3]),
...
y <- y[c(1,  16, 31,
  ...
  stargazer(y, summary = FALSE, out = "tabes/table_2_timi.htm",
