  ...
  # lapply(select(x, ends_with("FU")), function(x) table(is.na(x)))

  miss <- select(x, id, arm, ends_with("FU"))
  ...
  lvedv      <- lm(lvedv_FU          ~ arm + bsa.m2 + any.diabetes,
                   data = x) %>% tidy()
  lvesv      <- lm(lvesv_FU          ~ arm + bsa.m2 + any.diabetes,
                   data = x) %>% tidy()
  glvef      <- lm(glvef_FU          ~ arm +          any.diabetes,
                   data = x) %>% tidy()
  lvmass.a   <- lm(lvmass.a_FU       ~ arm + bsa.m2 + any.diabetes,
                   data = x) %>% tidy()
  latece     <- lm(latece_FU         ~ arm +          any.diabetes,
                   data = x) %>% tidy()
  sv.main    <- lm(sv.main_FU        ~ arm +          any.diabetes,
                   data = x) %>% tidy()
  ...
  # indexed
  lvedv.index      <- lm(lvedv.index_FU      ~ arm + any.diabetes,
                         data = x) %>% tidy()
  lvesv.index      <- lm(lvesv.index_FU      ~ arm + any.diabetes,
                         data = x) %>% tidy()
  lvmass.a.index   <- lm(lvmass.a.index_FU   ~ arm + any.diabetes,
                         data = x) %>% tidy()
  lv.mass.ed.index <- lm(lv.mass.ed.index_FU ~ arm + any.diabetes,
                         data = x) %>% tidy()
  lv.mass.es.index <- lm(lv.mass.es.index_FU ~ arm + any.diabetes,
                         data = x) %>% tidy()
  ...
  stargazer(y, summary = FALSE, out = "tables/table_unadjusted.htm",
